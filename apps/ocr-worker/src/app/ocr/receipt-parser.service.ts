...
import { buildReceiptExtractionPrompt, ExtractionContext } from './prompts/receipt-extraction.prompt';
...
export class ReceiptParserService {
  ...
  async parse(rawText: string, context: ExtractionContext): Promise<SmartReceiptResult> {
    this.logger.log('Sending raw OCR text to LLM for deep extraction...');

    try {
      const prompt = buildReceiptExtractionPrompt(context);
